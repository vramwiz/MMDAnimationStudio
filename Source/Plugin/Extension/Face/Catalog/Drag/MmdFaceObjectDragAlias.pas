unit MmdFaceObjectDragAlias;

// 選択PMXと表情から、AviUtl2の単一表情オブジェクトを生成する
// UTF-8エイリアスを構築・保存する。

interface

function TryBuildMmdFaceObjectAlias(const ModelFileName, FaceData: string;
  out AliasText: string): Boolean;
function TryWriteMmdFaceObjectAlias(const ModelFileName, FaceData,
...
function NormalizeFaceData(const Value: string;
...
begin
  Result := False;
  Normalized := '';
  if not TryDecodeMmdMorphSettingData(Value, Weights) then Exit;
...
var
  Lines: TStringList;
  NormalizedFaceData: string;
begin
  Result := False;
  AliasText := '';
  if (ModelFileName = '') or HasLineBreak(ModelFileName) or
    not TFile.Exists(ModelFileName) or
    not NormalizeFaceData(FaceData, NormalizedFaceData) then Exit;
  ...
  try
    ...
    Lines.Add('[0.0]');
    Lines.Add('effect.name=' + #$8868#$60C5);
    Lines.Add(#$30E2#$30C7#$30EB#$30D5#$30A1#$30A4#$30EB + '=' +
      ModelFileName);
    Lines.Add(#$8868#$60C5 + '=' + NormalizedFaceData);
    Lines.Add(#$8A2D#$5B9A + '=');
    Lines.Add('[0.1]');
...
begin
  Result := False;
  if (FileName = '') or not TryBuildMmdFaceObjectAlias(ModelFileName,
    FaceData, AliasText) then Exit;
  try
