...
- 通常サムネイルはVMD各トラックの先頭キーをPMXへ適用した静止画である。ホバー中だけ全ボーン／モーフキーを30fps基準で評価して同じ領域を動かし、離脱時は静止画へ戻す。VMD Bezierは内部データへ保持するが時間評価にはまだ使わない。エディタとD&Dは未実装で、Pluginはデータ受口だけの最小基盤とする。
